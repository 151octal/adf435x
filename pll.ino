// kd9fww. (c) 2024. ADF435x stand alone on the Arduino Nano. No, foolin'. Really!
  // hoo! hoo!
#include <SPI.h>
#include <ArxContainer.h>
  ...
  †† Faraday enclosures: one high value X7R between GND and EARTH IS better that a DC short.    */
// commented out, but wired:     D4                                      D11       D13
  /* Yes. The nano LED is in contention with the default SPI clock line and is not easily open
  circuited. Look, let it be and 'fugget about it', OK? SPI will work anyway regardless         */
enum class PIN : u8 {    /* MUX = 4, */ PDR = 6, LD = 7, LE = 10 /* DAT = 11, CLK = 13 */ };
auto wait4ld = []() { while( !digitalRead( static_cast<u8>(PIN::LD) )); }; // block until lock
...
auto tx(void *pByte, int nByte) -> void { // SPI stuff here
  ...
  digitalWrite(static_cast<u8>(PIN::LE), 0);
    // ascending Specification order (see below) <- speedy
...
enum Symbol : char {
  ...
  // ADF435x register layout / specification.
static constexpr struct { const u8 rank, offset, width; } Specification[] = {
  ...
  //          always flushed last (and always needs flushing to cause effect the pll).
...
struct SpecifiableStorageAndSPIOutput {
  ...
  struct Frame {
    ...
    // allocation and initialization ("reserved" & "contol" bit 'fields')
      ...
      // NB: this is an old school constructor. don't use the set() method to overwrite these bits
    u8 dirty; Buffer bfr; } frame = { 0, Frame::Buffer{ 0x180005, 4, 3, 2, 1, 0 } };
      // usage: object.set(symA,vX).set(symB,vY) ... ad infinitum
  auto set(Symbol symbol, u16 value) -> decltype(*this) {
    ...
    return *this; } // When it is appropriate to do so, flush() 'it' to the pll module.
      ...
      // which enables the motion of said train. (translation: flush() calls tx() which does SPI).
      // Rule 2). See: Rule 1).
  auto flush() -> void {  // NO, the type is auto. (It is NOT automatically triggered.)
    // to enable a flush to NOT be the last in a chain (but why?), use: -> decltype(*this)
    if( 0 == frame.dirty ) return;  // krude. needs refinement; use aggregate feature of tx()
...
SSSO global;     // it's as it's name implies
  ...
  /* {2404,258}[PGM,DATA] = {2848, 267} - {444, 9} bytes. <-- from the IDE: pasted below
  ...
  constexpr auto   fOSC{ 025e6 }, // manifest constants
                 ...
                 minPFD{ 125e3 },
                 maxPFD{ 050e6 };
constexpr enum SelectorSet { EVAL, CM23, WIDE, CM33, OOK, CM70, M2, M6 } Selector = M6;
...
// start here. add your tuning member and 'Selector' it or modify one of these
    ...
    [M6] = { 50e6, 6, 0 } };  // here output is fVCO divided by 64. see midX's decl below
...
auto setup() -> void  {
  ...
  digitalWrite(static_cast<u8>(PIN::LE), 1);
  pinMode(static_cast<u8>(PIN::LD), INPUT);
...
SSSO scratch;  // setup a (stack allocated) Specification configured SSSO
  ...
  scratch.set(Symbol::fraction, Fraction );
  scratch.set(Symbol::integer, Whole );
  scratch.set(Symbol::modulus, ModulusShort );
  scratch.set(Symbol::phase, 1);  // adjust phase AFTER loop lock
  scratch.set(Symbol::phase_adjust, Enab::OFF );
  scratch.set(Symbol::prescaler, preScale );
  scratch.set(Symbol::counterReset, Enab::OFF );
  scratch.set(Symbol::cp3state, Enab::OFF );
  scratch.set(Symbol::idle, Enab::OFF );
  enum PDpolarity { negative = 0, positive };
  scratch.set(Symbol::pdPolarity, PDpolarity::positive );
  enum LDPnS { ten = 0, six };
  scratch.set(Symbol::ldp, LDPnS::ten);
  enum LDF { fracN = 0, intN };
  scratch.set(Symbol::ldf, LDF::fracN );
  scratch.set(Symbol::cpIndex, 7);
    /*  ... can see no reason not to double buffer is because I am lacking the ... ? hmm.
        "There is no substitute for fundamental understanding." E.NevillePickering. Bradley U */
  scratch.set(Symbol::dblBfr, Enab::ON);
  scratch.set(Symbol::rCounter, Rcounter);
  scratch.set(Symbol::refDivider, oscToggler);
  scratch.set(Symbol::refDoubler, oscDoubler);
  constexpr enum MuxOut { HiZ = 0, DVdd, DGnd, rCntOut, nDivOut, aLock, dLock } Mux = HiZ;
  scratch.set(Symbol::muxOut, Mux);
  ...
  scratch.set(Symbol::LnLsModes, nsMode);
  constexpr auto ckDvdr = round(fOSC * 400e-6 / ModulusShort);
    static_assert(4096 > ckDvdr);
  scratch.set(Symbol::clkDivider, ckDvdr);
  constexpr enum ClockingMode { dividerOff = 0, fastLock, phResync } clockingMode = phResync;
  scratch.set(Symbol::clkDivMode, clockingMode);
  scratch.set(Symbol::csr, Enab::OFF );
  scratch.set(Symbol::chrgCancel, Enab::OFF );
  enum ABPnS { nS6fracN = 0, nS3intN };
  scratch.set(Symbol::abp, ABPnS::nS6fracN );
  scratch.set(Symbol::bscMode, 0);
  constexpr enum dBm { minus4, minus1, plus2, plus5 } rfPower = plus5;
  scratch.set(Symbol::rfOutPwr, rfPower );
  scratch.set(Symbol::rfOutEnable, Enab::ON );
  scratch.set(Symbol::auxOutPwr, dBm::minus4 );
  scratch.set(Symbol::auxOutEnable, Enab::OFF );
  scratch.set(Symbol::auxOutSelect, FeedBack::divided );
  scratch.set(Symbol::muteTillLD, Enab::ON );
  scratch.set(Symbol::vcoPwrDown, Enab::OFF );
  scratch.set(Symbol::bandSelectClkDiv, BscClkDiv );
  scratch.set(Symbol::rfDivSelect, RfDivisorTableIndex );
  scratch.set(Symbol::feedbackSelect, !Fdbk );  // Argh! why ( !FDBK ) ???
  constexpr enum LedMode { low = 0, lockDetect = 1, high = 3 } Led = lockDetect;
  scratch.set(Symbol::led_mode, Led);      
global = scratch; } // save, returning releases the function call stack
  // variable names declared therein go out of scope. 'Mux' is an example
  // this variable is declared for the sake of clarity.
global.flush(); /* "ship it!"
  ...
  leghorn[Foghorn]: "I say, I, I, I say, It's a joke son, S'posed ta laugh!"                    */
wait4ld();  // set phase at midrange: (180º)
global.set(Symbol::phase_adjust, Enab::ON).set(Symbol::phase, (ModulusShort >> 1)).flush();
}
